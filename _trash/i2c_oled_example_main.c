...
/*extern void example_lvgl_demo_ui(lv_disp_t *disp)
{
    ...
    lv_label_set_text(label, "20520688 - Ngo Ta Dinh Phong");
    lv_obj_set_width(label, 150);
    lv_obj_align(label, LV_ALIGN_TOP_MID, 0, 0);
}*/

extern void lv_example_img(lv_disp_t *disp)
{
	LV_IMG_DECLARE(uit);
	lv_obj_t * scr = lv_disp_get_scr_act(disp);
    lv_obj_t * uit_img = lv_img_create(scr);
    lv_img_set_src(uit_img, &uit);
    lv_obj_align(uit_img, LV_ALIGN_CENTER, 0, 0);
    lv_obj_set_size(uit_img, 64, 64);
}
